...
function BotNameData() {
	...
	$.ajax({
        ...
        // Add default option
        $('#idBotName').append('<option value="15">Select Deparment</option>');
			idBotName
...
function DepartmentData() {
	...
	$.ajax({
        ...
        // Add default option
        $('#idDepartmant').append('<option value="15">Select Deparment</option>');
...
function DashboardGridCreation(Obj) {
	...
	$.ajax({
		...
		data: JSON.stringify(postData),
		success: function(resultB) {
			//alert("Enable DataTable");


			//alert(resultB[0].ProcessId);
			//alert(resultB[0].StartTime);
			console.log(resultB)
			...
			for (var i = 1; i < rows.length; i++) { // Start from index 1 to skip header row
				var rowDate = new Date(rows[i].cells[8].innerText); // Assuming the date is in the ninth column

				if (isNaN(rowDate.getTime())) { // Check if the row date is invalid
					continue; // Skip if the row date is invalid
				}
